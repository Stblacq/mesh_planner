...
public:
  ...
  virtual std::vector<lvr2::VertexHandle> findMinimalCostPath(const mesh_map::Vector& original_start, const mesh_map::Vector& original_goal);
  virtual void publishPath(const ros::Publisher &path_pub, const std::vector<lvr2::VertexHandle> &path,std::vector<geometry_msgs::PoseStamped>& plan);
